...
@implementation WaveGridArrowDown
...
- (void)setPercent:(CGFloat)percent {
    ...
    _currentLinePointY = _originY + height * (1.f - fabs(percent));
}
...
- (void)drawRect:(CGRect)rect {
    ...
    for (NSUInteger i = 0; i < 11 * 7; i++) {
        NSUInteger line = i / 7;
        NSUInteger row = i % 7;
        CGPathAddRect(pathBlock, nil, CGRectMake(_originX + _widthAddOne * 2 + _widthAddOne * row, _originY + _widthAddOne * line, _width, _width));
    }
    
    // 2.上面的由方块组成的倒三角型 11 * 6
    ...
    for (NSUInteger i = 0; i < 6 * 11; i++) {
        ...
        if (abs((int)(i - middleNum)) <= (6 - line - 1)) {
            CGPathAddRect(pathBlock, nil,
                          CGRectMake(_originX + _widthAddOne * ((int)(i - middleNum) + 5), _originY + _widthAddOne * 11 + _widthAddOne * line, _width, _width));
        }
    }
